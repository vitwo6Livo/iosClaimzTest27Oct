...
class ClaimzStatusCardShimmer extends StatelessWidget {
  final String title;
  final int number;

  ClaimzStatusCardShimmer({Key? key, required this.title, required this.number})
      : super(key: key);
          ...
          child: Column(
            children: [
              Container(
                  ...
                  height: 110,
                  child:
                      _buildElevationDoughnutChart(title, number.toString())),
              Text(
                title,
  ...
  SfCircularChart _buildElevationDoughnutChart(String title, String number) {
    return SfCircularChart(
      ...
      series: _getElevationDoughnutSeries(
          int.parse(number.toString()), 100 - int.parse(number.toString())),
    );
  }
